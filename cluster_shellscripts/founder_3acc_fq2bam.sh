#!/bin/bash
#SBATCH --job-name=founder_bwa_3acc  # Name of job
...
#SBATCH --time=24:00:00              # Time limit in hrs:min:sec (default for sjmac queue is 8 hrs)
#SBATCH --output=./run_log_files/found_bwa_3acc_%j.log     # Standard output and error log
#SBATCH --array=1-15                 # Set number (N) of parallel jobs (1-N)
...
dir1="process"
dir2="bam"
files="readname.mapping.3acc.txt"
...
# Col6/7 = Read1/Read2 of sequencing replicate "c"
...
F3=`head -n $SLURM_ARRAY_TASK_ID $files | tail -n 1 | cut -f6`
R3=`head -n $SLURM_ARRAY_TASK_ID $files | tail -n 1 | cut -f7`

# DIFFERS from "founder_2acc_fq2bam.sh" in file to be read in, the splitting of
# the readname file, that 3 instances of bwa are run, and the merging
...
bwa mem -t 4 -M $ref $F3 $R3 | samtools view -bS - > $dir1/$shortname.3.temp.bam
samtools merge -f $dir1/$shortname.temp.bam $dir1/$shortname.1.temp.bam $dir1/$shortname.2.temp.bam $dir1/$shortname.3.temp.bam
